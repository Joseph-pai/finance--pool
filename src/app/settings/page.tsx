...
export default function SettingsPage() {
  ...
  const [form, setForm]         = useState({ display_name: '', email: '', password: '', role: 'member' as UserRole });
        ...
        <div className="flex items-center gap-4">
          ...
          <div style={{ flex: 1 }}>
            ...
            <div className="text-sm text-secondary">
              {profile?.role === 'admin' ? '🛡️ 系統管理員' : 
               profile?.role === 'lake_manager' ? '🌊 湖泊管理員' : '👤 家庭成員'}
            </div>
          </div>
        </div>
      </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted">
                        {m.role === 'admin' ? '🛡️ 系統管理員' : 
                         m.role === 'lake_manager' ? '🌊 湖泊管理員' : '👤 家庭成員'}
                      </div>
                    </div>
                  </div>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
              ...
              <div className="form-group">
                ...
                <select id="settings-form-role" className="form-input form-select" value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value as UserRole }))}>
                  <option value="member">👤 家庭成員 (一般權限)</option>
                  <option value="lake_manager">🌊 湖泊管理員 (財務審批)</option>
                  <option value="admin">🛡️ 系統管理員 (最高權限)</option>
                </select>
              </div>
