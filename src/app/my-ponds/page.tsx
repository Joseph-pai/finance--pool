...
export default function MyPondsPage() {
  ...
  const handleTransferToPondB = async () => {
    ...
    try {
      // 1. 減少 Pond A
      const newA = Math.max(0, pondA.current_balance - amt);
      await supabase.from('pond_a').update({ current_balance: newA }).eq('id', pondA.id);

      // 2. 增加 Pond B (使其趨於零)
      const newB = Math.min(0, pondB.current_balance + amt);
      await supabase.from('pond_b').update({ current_balance: newB }).eq('id', pondB.id);
        ...
        <div className="modal-overlay" onClick={() => setShowInjectModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 400 }}>
            ...
            <div style={{ marginBottom: 'var(--space-5)' }}>
              <p className="text-sm text-secondary">將收入池資金移轉至支出池，減少個人支出水位。</p>
              <div className="flex justify-between text-xs font-mono" style={{ marginTop: 12, padding: 8, background: 'rgba(0,0,0,0.2)', borderRadius: 4 }}>
                <span>可用：{formatTWD(pondA?.current_balance ?? 0)}</span>
                <span>支出：{formatTWD(bBalance)}</span>
              </div>
            </div>
            <div className="form-group" style={{ marginBottom: 'var(--space-6)' }}>
              <label className="form-label">注入金額</label>
              <input type="number" className="form-input" placeholder="0" value={injectAmount} onChange={e => setInjectAmount(e.target.value)} autoFocus />
            </div>
            <div className="flex gap-3" style={{ justifyContent: 'flex-end' }}>
              <button className="btn btn-ghost" onClick={() => setShowInjectModal(false)}>取消</button>
              <button className="btn btn-primary" onClick={handleTransferToPondB} disabled={injecting || !injectAmount || Number(injectAmount) <= 0}>
                {injecting ? '處理中...' : '確認注入'}
              </button>
            </div>
          </div>
